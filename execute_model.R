...
for (i in 1:nrow(cpds)) {
  ...
  CpdConstants <-
    init_Compound_Paras(UConstants, SysConstants, CpdConstants)
...
for (i in seq(-200, 1000, 2)) {
  ...
  results <-
    rbind(results, cbind(as.data.table(CpdConstants), as.data.table(t(C))))
}

show(
  ggplot(results, aes(log_Pow, CellU)) +
    geom_point(color = 'red') + geom_hline(yintercept = CpdConstants$CNom, linetype = 2) +
    # geom_point(aes(y = Plastic), color = 'blue', size = 1) +
    scale_y_log10() + ggtitle('Cell Concentration vs logPow')
)
